...
export default async function Post({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><Header/><main className="section"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify([articleSchema, faqSchema]) }} /><article className="container" style={{maxWidth:900}}><p className="eyebrow">{site.brand} guide</p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p><div className="card"><h2>Key takeaways</h2><ul className="list">{post.takeaways.map((item)=><li key={item}>{item}</li>)}</ul></div>{post.sections.map((section)=><section className="article-block" key={section.heading}><h2>{section.heading}</h2><p>{section.body}</p></section>)}<section className="article-block"><h2>Local vs remote decision table</h2><div className="table"><div className="row"><b>If the task needs...</b><b>Lean toward...</b><b>Why</b></div><div className="row"><span>Office presence, errands, paper files</span><span>Local assistant</span><span>The work cannot be checked fully online.</span></div><div className="row"><span>Inbox, calendar, CRM, support replies</span><span>Remote assistant</span><span>The work is tool-based and easy to review.</span></div><div className="row"><span>Recurring tasks that need broad daily coverage</span><span>Overseas VA or managed service</span><span>Distance matters less when the instructions and review rules are clear.</span></div></div></section><section className="article-block card"><h2>Question to copy for a provider call</h2><p className="quote small">"{post.script}"</p></section><section className="article-block"><h2>Check the sources</h2><p>Use these public references when you compare wage data, worker classification, or advertising claims.</p><ul className="list">{publicSources.map((source)=><li key={source.url}><a className="text-link" href={source.url}>{source.name}</a>: {source.note}</li>)}</ul></section><section className="article-block"><h2>Before you contact a provider</h2><p>Write the task list first. Ask each provider who screens the assistant, who handles training or replacement, and how the work is reviewed.</p><p><a className="text-link" href="/contact">Build an assistant role brief →</a></p></section></article><CTA/></main><Footer/></>;
}
